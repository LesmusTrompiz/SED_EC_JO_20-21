...
void TIMER3_IRQHandler(void)
{
	static uint32_t t = 0;
	char msg [20];
	if(((LPC_TIM3->IR>>0)&1)==1)   //El MR0 interrumpe y reconfiguramos para que CAP2.0 interrumpa a flanco de subida
	{
		LPC_TIM3->IR=1<<0;
	...
	else if(((LPC_TIM3->IR>>4)&1)==1)  //El CAP2.0 interrumpe y guarda el valor en CR0 automaticamente pasandlo a temp y reconfiguramos para que interrumpa a flanco de bajada
	{
		LPC_TIM3->IR=1<<4;
		
		if((LPC_TIM3->CCR>>0)&1==1)
		{
			t = LPC_TIM3->CR0;
