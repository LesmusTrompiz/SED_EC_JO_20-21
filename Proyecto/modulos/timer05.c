...
void TIMER0_IRQHandler()
{
  ...
  */
  ...
  static int  ciclo = 0;                              // Static variable that indicates how much cycles have
  ...
  switch(sonar.state)
  {
    ...
    case(ST_AUTOMATICO):                              // If the sonar is in Automatic mode:
      ...
      if(!sonar.f_block_move                          // If the servo is allowed to move
         &&                                           // AND
        (ciclo >= sonar.servo_period))                // the cycle coincides with the servo's period. 
      {	
        ...
        ciclo = 0;                                    // Reset the cycle counter.
      }
      break;
